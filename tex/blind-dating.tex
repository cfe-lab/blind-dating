...
For these data, we generated an additional 450 subjects with 100 sequences each, bringing the total amount of simulated data to 500 subjects.
...
We evaluated the latency of the censored sequences using two methods. The first was a {binomial model} and the second was a {GLME model}.

For the {binomial model}, we used R's binom.test function fit the ocurrence of sequences with positive difference (i.e. latency) to a binomial model. We used $p=0.5$ as the null model.
...
The {binomial model} for the \emph{plasma} data gave $p$-value of $0.1468$ and a predicted success probability of $0.5392$ with a confidence interval of $(0.4792, 1)$.
...
The {binomial model} for the \emph{mixed} data gave $p$-value of $0.02263$ and a predicted success probability of $0.5294$ with a confidence interval of $(0.5052, 1)$.
...
\begin{figure}[ht]
	...
	\caption[Latency Model]{An illustration of the latency model. On a tip with branch length $S$, we randomly select a become unlatent period, $U$, and then random select a become latent period, $L$, between the interval $S-U$ and $S$.}
