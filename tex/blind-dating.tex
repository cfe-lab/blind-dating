...
\usepackage{comment}
\usepackage{caption}
\usepackage[labelfont=bf,textfont=bf]{subcaption}
\usepackage{mathptmx}
\usepackage{nicefrac}
\usepackage{authblk}
\usepackage{array}
...
\begin{table*}
\tabletitle{Patient sequence data}
...
\end{tabular}
\end{center}
  \caption{Details of the linear models applied to the real data sets. RMSD is the root mean squared deviance. Patients where we were unable to reject the null hypothesis are marked in bold.
...
\begin{figure}[p]
	\figtitle{Latency scheme}
	\centering
	\includegraphics{figures/latency-scheme}
	\caption[Latency Scheme]{
	Reconstructing the time that a viral lineage entered a latent state from sequence variation. A dashed line illustrates the linear relationship between the divergence of lineages from the ancestral sequence at the root ($y$-axis) and passage of time since the root ($x$-axis).
...
\begin{figure}[p]
	\figtitle{Latency model}
	\centering{}
	\includegraphics[width=10cm]{figures/latency-model}
	\caption[Latency Model]{
	An illustration of the model simulating viral latency.
...
\begin{figure}[p]
	...
	\end{subfigure}
	\caption[Simulated Data]{Plots from the simulated data set of the evolutionary distance vs time.
...
\begin{figure}[p]
	...
	\end{subfigure}
	\caption[Difference Density]{The densities of the scaled difference of each patient in the real data sets.
...
\begin{figure}[p]
	\figtitle{Patient 16617 from the \emph{mixed} data set}
	\centering
	\includegraphics[width=12cm]{figures/patient_16617.pdf}
	\caption[Treament]{Same as Figure \ref{fig:results1} for Patient 16617 from the \emph{mixed} data set. The time between the green bars represents when the patient underwent therapy.}
	\label{fig:treatment}
\end{figure}
