...
Thus if the frequency of censored lineages whose difference between sample and predicted date are positive (i.e.~are sampled later than the regression predicts) is significantly greater than a half, then we expect that some of the censored lineages have latency.
However is the frequency of censored lineages whose difference between sample and predicted date are positive is close to a half, then the censored lineages behave similar to the uncensored lineages suggesting that there is little or no latency in the censored lineages.
...
We applied our method to simulated phylogenies without viral latency incorporated. After censoring 50\% of the tips, we attempted to reconstruct their dates. The scaled RMSD of the reconstructed dates was $4.5 \times 10^{-2}$ and the scaled median difference $-6.7 \times 10^{-4}$ days. A nonparametric binomial test on the sign of the difference between sampled and reconstructed dates agrees that there is no latency (p=0.74).
...
The total scaled RMSD of the extreme simulated model with viral latency was $3.3 \times{10}^{-2}$ days and the scaled median difference was $5.6 \times 10^{-2}$. The total scaled RMSD of the simulated models with viral latency from the first set was $6.5 \times 10^{-2}$ days and the scaled median difference was $2.3 \times 10^{-2}$ days. The unscaled median difference was $51.$ days compared to the actual median latency of $35.$ days.
...
2 of the individuals from the {\em plasma} data set were screened as (inconclusive) when applying the log ratio test to their date reconstruction.
See Figure \ref{fig:resultsancre} for an example of the date reconstruction from the \emph{plasma} data.
...
See Supplementary Materials (ref) for details pn the results using the outgroup rooting.
...
Applying the binomial test to only the individuals who received treatment gave a lower predicted success probability and significantly higher $p$-value (0.45 and 0.98 respectively). This suggests that there are few latent lineages in the treated patients.
