...
Finaly, the root-to-tip distances of possibly latent sequences are mapped to the optimal regression line to estimate reservoir establishment dates.
...
When sampling dates are known however, the corresponding tips of the phylogeny can be fixed at these timepoints, allowing the rate of evolution be estimated directly \citep{Rodrigo99}.
...
For our experiments we compiled 6 data sets of real and synthetic longitudunal HIV sequences.
...
We estimated $l$ from the frequency of latent cells found by \cite{Ho13} ($i = 3. \times 10^{-4}$ day$^{-1}$).
...
For these data, we generated an additional 150 subjects with 100 sequences each, bringing the total amount of simulated data to 200 subjects and 2000 sequences.
...
Finally we applied a nonparamatric binomial test to detect the presence of latency in the data set.
...
Next we applied the method to simulated phylogenenies with 50\% of the tips containing viral latency.
...
In Figure \ref{fig:resultslatent}, we have an example of the date reconstruction of a simulated patient with viral latency incorportated from the \emph{simulated (latency 1)} data set.
Each HIV sequence from the simulated patient is plotted on a genetic distance versus collection time plot.
...
We can see that most of the `latent' sequences appear to the right of the regression line and some are ().
...
Figure \ref{fig:resultsancre} shows an example of the date reconstruction from the \emph{plasma} data set. We can see that in the real data the points do fit around the regression line as well as in the simulated data. (More)
...
While on therapy the patient's viral load remained above or below the detectable limit of the study (50 copies/ml).
However, after treatment was halted 439 days after the onset of therapy the viral load of the patient quickly returned to its previous value before therapy was initated \citep{Llewellyn06}.
...
Due to their relative position to the line of regression, we believe that the (circled) points have undergone latency.
...
(expand strengths and weakness)
Bayesian methods to estimate unknown sequence dates \citep{Shapiro11} may be more acurrate, but require more computation time (and resources).

%\section * {Conclusion} \label{sec:conclusion}
%There are ways in which this method could be improved. 
There are many ways that this method could be improved.
...
\begin{figure}[ht]
	...
	If the relationship between sequence divergence and time is sufficiently linear, then the time between l;atency establishment and sampling, here represented by the thick red dashed line, can be inferred from its sequence.
...
\begin{figure}[ht]
	...
	\end{subfigure}
	\begin{subfigure}[ht]{8cm}
		\includegraphics[width=8cm]{figures/lanl_annot.pdf}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=12cm]{figures/patient_16617_annot.pdf}
	\caption{Patient 16617 from the \emph{mixed} data set. The time between the green bars represents when the patient underwent therapy.}
	\label{fig:treatment}
\end{figure}
...
\emph{simulated (extreme latency)} & $3.3 \times 10^{-2}$ & $-5.6 \times 10^{-2}$ \\ % & $-70.$ & $90.$ \\
\emph{simulated (latency 1)} & $6.5 \times 10^{-2}$ & $-2.3 \times 10^{-2}$ \\ % & $-51.$ & $35.$ \\
