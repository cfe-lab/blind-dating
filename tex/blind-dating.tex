...
\author[2,3]{Jeffrey B. Joy} % jjoy@cfenet.ubc.ca
...
Similarly, we parametrized $i$ from the frequency of latent cells found by \cite{Ho13} ($i = 3. \times 10^{-4}$ day$^{-1}$).
...
We have found evidence of viral latency from the reconstructed dates of HIV-1 integration within hosts using a simple new method of phylogenetic analysis of sequence variation.
Using simulated data sets, we verified that this method could accurately recover integration dates from the phylogeny.
This approach can provide an important resource for the study of HIV latent reservoirs and the development of effective cure strategies.

%We introduced a pipeline which estimates the integration dates of HIV-1 lineages within-host given the assumption of a strict molecular clock.
%We showed how this pipeline can be used to accurately recover the dates when latent lineages were incorporated into the HIV-1 reservoir.
%It is our hope that the ability to estimate the dates of HIV integration in the latent reservoir will aid in the development of more effective cure strategies of HIV.
