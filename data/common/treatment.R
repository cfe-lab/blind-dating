...
##
		...
		if (size) {
			...
			l.x <- if (other) {
					...
					plot.xlim[2] - (plot.xlim[2] - plot.xlim[1])*.375
				}
			y.intersp=1.1
			cex=1.5
		} else {			
			l.x <- plot.xlim[2] - (plot.xlim[2] - plot.xlim[1])*.25
			y.intersp=1
			cex=1
		}
				
		plot(plasma.dates, plasma.dists, xlab="Time (days)", ylab=sprintf("Divergence from root"), xlim=plot.xlim, ylim=plot.ylim,  pch=20, cex.lab=1.5, cex.axis=1.2, col='black', cex=cex)
		...
		if (data.type == 2) {		
			...
			if (size) {
				l.y <- if (other) {
						...
						(plot.ylim[2] - plot.ylim[1])*.19 + plot.ylim[1]
					}	
			} else {
				l.y <- (plot.ylim[2] - plot.ylim[1])*.13 + plot.ylim[1]
			}
					
			legend(l.x, l.y, c("Uncensored dates", "Latency dates", "Censored dates"), col = c("black", "red", "red"), pch = c(20, 18, 5), y.intersp=y.intersp, cex=cex)
		} else {
			if (size) {
				l.y <- if (other) {
						plot.ylim[2]
					} else {
						(plot.ylim[2] - plot.ylim[1])*.13 + plot.ylim[1]
					}
			} else {
				l.y <- (plot.ylim[2] - plot.ylim[1])*.13 + plot.ylim[1]
			}
		
			legend(l.x, l.y, c("RNA dates", "DNA dates"), col = c("black", "red"), pch = c(20, 5), y.intersp=y.intersp, cex=cex)
		}
