...
do.dup <- function(f, info) {
	same <- lapply(
		f,
		function(x)
			which(unlist(lapply(f, function(y) all(x == y))))
	)
	...
	o <- order(
		if (use.real)
			as.numeric(as.Date(info[, weight]))
		else
			info[, weight], decreasing=!use.lowest
	)
	info <- info[o, ]
	...
	dups <- do.call(
		...
		lapply(
			same,
			function(x)
				lapply(1:length(x), function(y) info$FULLSEQID[x[-y]])
		)
	)
	rep.dup <- unlist(
		lapply(same, function(x) rep(info$FULLSEQID[x[1]], length(x)))
	)
...
op <- add_option(op, "--type", type='logical', action='store_true', default=F)
...
if (type) {
	...
	f <- do.call(c, lapply(res, `[[`, "f"))
	info <- as.data.frame(do.call(rbind, lapply(res, `[[`, "info")))
} else {
	res <- do.dup(f, info)
	f <- res$f
	info <- res$info
