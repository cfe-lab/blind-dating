...
stats <- data.frame(
	...
	bin.test=binom.test(sum(data[data$censored == 1, "date.diff"] >= 0), sum(data$censored == 1), alternative='less')$p.value,
	t.test=t.test(data[data$censored == 1, "date.diff"], alternative='less')$p.value
)
stats.col.names <- c(
	"Patient",
	"Training Samples",
