...
pdf.options(family="Helvetica", fonts="Helvetica", width=7, height=4, colormodel='rgb')
...
p <- p + scale_x_date(name="Collection Year", breaks=as.Date(paste0(seq(1996, 2017, by=5), "-01-01")), labels=seq(1996, 2017, by=5))
...
p <- p + geom_line(size=.5) 
p <- p + scale_y_log10(name="Viral load", breaks=10^c(1, 3, 5), labels=sapply(c(1, 3, 5), function(x) bquote(''*10^{.(x)}*'')))
p <- p + scale_x_date(name="Collection Year", breaks=as.Date(paste0(seq(1997, 2017, by=5), "-01-01")), labels=seq(1997, 2017, by=5))
...
p <- ggplot(subset(data, Censored == 1), aes(x=Estimated.Date, fill=factor(as.Date(Collection.Date, origin="1970-01-01"), levels=c("2007-09-20", "2011-07-19", "2015-09-30", "2016-06-29")))) +
	geom_rect(xmin=as.numeric(as.Date("1997-08-01"), origin="1970-01-01"), xmax=as.numeric(as.Date("1997-11-01"), origin="1970-01-01"), ymin=0, ymax=6, fill=THERAPY_COLOUR) +
	geom_rect(xmin=as.numeric(as.Date("2006-08-01"), origin="1970-01-01"), xmax=Inf, ymin=0, ymax=6, fill=THERAPY_COLOUR) +
	geom_linerange(x=as.numeric(as.Date("1996-08-12"), origin="1970-01-01"), ymin=0, ymax=6) +
	...
	scale_x_continuous(name="Estimated integration year", breaks=as.numeric(as.Date(paste0(seq(1994, 2011, by=4), "-01-01"))), labels=c("1994", "1998", "2002", "2006", "2010")) +
	scale_y_continuous(name="Frequency", breaks=c(0, 2, 4, 6), limits=c(0, 9)) +
...
p <- ggplot(subset(data, Censored == 1), aes(x=Estimated.Date, fill=factor(as.Date(Collection.Date, origin="1970-01-01"), levels=c("2013-03-15", "2016-08-15")))) +
	geom_rect(xmin=as.numeric(as.Date("2000-08-01"), origin="1970-01-01"), xmax=as.numeric(as.Date("2006-09-01"), origin="1970-01-01"), ymin=0, ymax=3, fill=THERAPY_COLOUR2) +
	geom_rect(xmin=as.numeric(as.Date("2006-09-01"), origin="1970-01-01"), xmax=Inf, ymin=0, ymax=3, fill=THERAPY_COLOUR) +
	geom_linerange(x=as.numeric(as.Date("1997-02-11"), origin="1970-01-01"), ymin=0, ymax=3) +
	...
	scale_x_continuous(name="Estimated integration year", breaks=as.numeric(as.Date(paste0(seq(1991, 2010, by=4), "-01-01"))), labels=c("1991", "1995", "1999", "2003", "2007")) +
	scale_y_continuous(name="Frequency", breaks=c(0, 1, 2, 3), limits=c(0, 4.5)) +
